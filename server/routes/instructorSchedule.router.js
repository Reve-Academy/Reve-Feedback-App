...
router.get('/weeks/', (req, res) => {
    ...
    let queryText = 'SELECT * FROM weeks WHERE program_id = $1 ORDER BY "number" ASC;' ; 
...
router.put('/', (req, res) => {
    if(req.isAuthenticated()){
        ...
        let focusQueryText = `UPDATE focus SET "x" = $1, "y" = $2, "w" = $3, "h" = $4 WHERE "id" = $5;`;
        //values to be inserted into focus
        focusValues = [layout[j].x, layout[j].y, layout[j].w, layout[j].h, layout[j].i];

        pool.query(focusQueryText, focusValues).then((result) => {
            res.sendStatus(200);
        }).catch((err) => {
